...
| --- | --- | --- | --- | --- |
| **Codex** | `codex` | `codex --ask-for-approval never exec --json --sandbox workspace-write --skip-git-repo-check --cd <dir> -` (prompt on stdin; `--ask-for-approval` is a global flag and must be before `exec`, or omitted — headless `codex exec` already defaults approval to never) | [Codex CLI](https://developers.openai.com/codex/cli): `codex login` (ChatGPT). Do not use `OPENAI_API_KEY` for Codex. | `codex login status` (exit 0 when signed in) |
